...
##freq downloaded is baed on GRCh38, need to be converted to GRCh37
from pyliftover import LiftOver
lo = LiftOver('hg38', 'hg19')
fw=open("/work/long_lab/qli/ref/dbSNP/population_frequency/latest_release/freq.hg19.vcf","w")
log=open("/work/long_lab/qli/ref/dbSNP/population_frequency/latest_release/freq.hg19.vcf.log","w")
with open("/work/long_lab/qli/ref/dbSNP/population_frequency/latest_release/freq.vcf","r") as fr:
    ...
    while line:
        ...
        if line.startswith("#"):
            ...
            if (line_list[0] in hg38_ref_sequences):
                old_chr=str(hg38_ref_sequences.index(line_list[0])+1)
                old_position=line_list[1]
                new=lo.convert_coordinate("chr"+old_chr, int(old_position))
                if(len(new)>0):
                    line_list[1]=str(new[0][1])
                    line_list[0]=hg19_ref_sequences[hg38_ref_sequences.index(line_list[0])]
                    line_out="\t".join(line_list)
                    fw.write(line_out+"\n")
                else:
                    log.write("Cannot convert "+line+"\n")
            else:
                log.write("Not in hg38_ref_sequences "+line+"\n")
        line=fr.readline().strip()
fw.close()
log.close()

##Step 2： Generate pickle files containing rs number and MAF
fr=open("/work/long_lab/qli/ref/dbSNP/latest_release/VCF/hg19_ref_sequences.pickle","rb")
hg19_ref_sequences = pickle.load(fr)
fr.close()
cur_chr_index=0
cur_chr=hg19_ref_sequences[cur_chr_index]

info_dict={}
#CHROM  POS     ID      REF     ALT     QUAL    FILTER  INFO    FORMAT  SAMN10492695
with open("/work/long_lab/qli/ref/dbSNP/population_frequency/latest_release/freq.hg19.vcf","r") as fr:
    line=fr.readline()
    while line:
        if not line.startswith("#"):
            line_list=line.strip().split("\t")
            if (line_list[0] in hg19_ref_sequences) and (line_list[0] == cur_chr):
                if(len(line_list)==21):
            ...
            elif (line_list[0] in hg19_ref_sequences):
                ###end of one chr
                fw=open("/work/long_lab/qli/ref/dbSNP/population_frequency/latest_release/hg19."+cur_chr+".pickle","wb")
...
cur_chr_variant_id_2_rs_dict=pickle.load(open("hg19."+str(cur_chr)+".pickle","rb"))
